<!--
  CloseConfirmModal.svelte — Модалка подтверждения закрытия
-->
<script>
  import { invoke } from "@tauri-apps/api/core";

  let { show = false, onClose = () => {}, playClickSound = () => {}, playHoverSound = () => {} } = $props();
...
{#if show}
  ...
  <div class="absolute inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in" role="presentation" onclick={onClose}>
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div role="dialog" tabindex="0" aria-labelledby="close-dialog-title" aria-modal="true" class="bg-[#1b1b1b] border border-white/10 rounded-xl px-8 py-6 max-w-xs w-full shadow-2xl" onclick={(e) => e.stopPropagation()} onkeydown={(e) => { if (e.key === "Escape") onClose(); }}>
